...
function CustomCursor() {
  ...
  const cursorStyle = {
    zIndex: 2147483647, // 💥 ABSOLUTE TOP (max safe z-index)
  ...
  return (
    <>
      ...
      <div
        ...
        aria-hidden="true"
        style={cursorStyle}
        className={`pointer-events-none fixed left-0 top-0 h-12 w-12 rounded-full border transition-opacity duration-200 ${followerClass}`}
      />
      ...
      <div
        ...
        aria-hidden="true"
        style={cursorStyle}
        className={`pointer-events-none fixed left-0 top-0 h-2.5 w-2.5 rounded-full ${dotClass}`}
      />
    </>
  );
}
